...
namespace Forge.Tests
{
    ...
    public class ShadowRulesTests
    {
        ...
        public void 표는_정본_자리를_그대로_쥔다()
        {
            ...
            ShadowTable t = T();
            Assert.AreEqual(29, t.Count, "자(check_box_shadows)가 세는 자리와 같은 수여야 한다 — 36회차 equipcell_drop 28 · 38회차 pettile_drop 29");
        ...
        public void 딱딱한_턱과_흐린_그림자로_갈린다()
        {
            ...
            Assert.AreEqual(22, soft, "흐린 그림자(36회차 equipcell_drop 21 · 38회차 pettile_drop 22) — 29회차에 남은 열하나의 **값을 먼저 재** 표에 담았다(배선은 그 파일의 lock 이 풀리는 회차 · 자의 KNOWN 이 그 목록이다)");
