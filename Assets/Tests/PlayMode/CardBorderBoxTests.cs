...
{
    /// <summary>
    /// T465(세로)·T473(가로) — 클론이 CSS `padding` 을 테두리 **바깥**에서 재던 것을 바로잡았다: 팝업 카드(`PopupKit.Card` · `DungeonPopups.Card`)의 rect 는
    /// 이제 CSS 의 **패딩 상자**이고 테(`line`/`bg`)·면·그늘은 그 rect 보다 **사방** `line3_px` 씩 **밖에** 선다(`PopupKit.Grow`).
    ...
    public class CardBorderBoxTests
    {
        ...
        /// <summary>테 상자가 «부모를 채우되 **사방** line3 만큼 밖» 인지(T465 세로 · T473 가로).</summary>
        static void AssertGrown(RectTransform rt, float line3, string what)
        {
            ...
            Assert.AreEqual(-line3, rt.offsetMin.x, 0.01f, what + " 왼쪽으로 line3 만큼 밖(T473)");
            Assert.AreEqual(line3, rt.offsetMax.x, 0.01f, what + " 오른쪽으로 line3 만큼 밖(T473)");
        }

        [UnityTest]
        public IEnumerator 공용_카드는_rect_가_패딩_상자고_테_면_그늘이_사방_line3_만큼_밖에_서며_X_는_그_몸의_아래변에_걸린다()
        {
            yield return Boot();
            ...
            AssertGrown(line, line3, "테");
            // 면 = 테 안쪽 = rect 그대로(T465 세로 · T473 가로)
            Assert.AreEqual(0f, face.offsetMin.y, 0.01f, "면 아래끝 = rect 아래끝"); Assert.AreEqual(0f, face.offsetMax.y, 0.01f, "면 위끝 = rect 위끝");
            Assert.AreEqual(0f, face.offsetMin.x, 0.01f, "면 왼끝 = rect 왼끝(T473)"); Assert.AreEqual(0f, face.offsetMax.x, 0.01f, "면 오른끝 = rect 오른끝(T473)");
            Rect rc = World(card), rl = World(line), rf = World(face);
            Assert.AreEqual(rc.height, rf.height, rc.height * 0.002f, "면 높이 = rect 높이");
            Assert.AreEqual(rc.width, rf.width, rc.width * 0.002f, "면 폭 = rect 폭(T473)");
            Assert.AreEqual(rc.height * (1f + 2f * line3 / card.rect.height), rl.height, rc.height * 0.002f, "카드 몸(테) 높이 = rect + 2·line3");
            Assert.AreEqual(rc.width * (1f + 2f * line3 / card.rect.width), rl.width, rc.width * 0.002f, "카드 몸(테) 폭 = rect + 2·line3(T473)");
            Assert.Greater(rl.yMax, rc.yMax, "테 위끝이 rect 위끝보다 위"); Assert.Less(rl.yMin, rc.yMin, "테 아래끝이 rect 아래끝보다 아래");
            Assert.Less(rl.xMin, rc.xMin, "테 왼끝이 rect 왼끝보다 왼쪽(T473)"); Assert.Greater(rl.xMax, rc.xMax, "테 오른끝이 rect 오른끝보다 오른쪽(T473)");
            ...
            Assert.AreEqual(2f * line3, lipRt.offsetMax.x - lipRt.offsetMin.x, 0.01f, "턱의 가로 여분 = 2·line3(좌우 line3 씩 · T473)");
            ...
            Assert.AreEqual(UiKit.L("profile_w") * UiKit.RefW, card.rect.width + 2f * line3, 0.5f, "카드 몸 폭 = 표 profile_w(T473 — rect 는 그보다 테 두 겹 좁다)");
        ...
        public IEnumerator 던전_계열_카드도_테가_rect_밖에_서고_첫_자식은_테_안쪽에서_pad_만큼_떨어진다()
        {
            ...
            Assert.AreEqual(rc.xMin, rf.xMin, rc.width * 0.002f, "면 왼끝 = rect 왼끝(테 안쪽 · T473)");
            Assert.AreEqual(rc.xMax, rf.xMax, rc.width * 0.002f, "면 오른끝 = rect 오른끝(T473)");
            Assert.AreEqual(UiKit.RefW * UiKit.L("card_w"), card.rect.width + 2f * line3, 0.5f, "카드 몸 폭 = 표 card_w(T473 — rect 는 테 두 겹 좁다)");
