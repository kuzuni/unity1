...
namespace Forge.Tests.PlayMode
{
    ...
    public class ProfileCardTests
    {
        ...
        public IEnumerator 프로필_카드는_정본_보정값_없이_앱_가운데에_선다()
        {
            ...
            Assert.AreEqual(UiKit.L("profile_w") * UiKit.RefW - PopupKit.Line3 * 2f, card.rect.width, 0.5f, "카드 rect(패딩 상자) = 표(profile_w · 카드 몸) − 테 두 겹(T473)");
        ...
        public IEnumerator 리그_보상_카드는_정본_보정값_없이_앱_가운데에_선다()
        {
            ...
            Assert.AreEqual(0.5f, card.anchorMin.y, 1e-4f); Assert.AreEqual(0.5f, card.pivot.y, 1e-4f, "가운데 앵커·피벗");
            Assert.AreEqual(UiKit.L("modal_wide_w") * UiKit.RefW - PopupKit.Line3 * 2f, card.rect.width, 0.5f, "카드 rect(패딩 상자) = 표(modal_wide_w · 카드 몸) − 테 두 겹(T473) — 표는 건드리지 않는다");
