...
namespace Forge.Tests.PlayMode
{
    ...
    /// **여덟 줄**만 들어 «차단 목록»·«개인정보 보호» 가 첫 화면에서 사라졌다(검수 Q · 런 284 실측).
    ...
    public class SettingsRowTests
    {
        ...
        public IEnumerator 설정_행_높이는_정본_4_75퍼센트H_이고_첫_화면에_열_줄이_든다()
        {
            ...
            // 정본은 이 상자에 열 줄이 다 들고 열한째가 반쯤 잘린다 — 열 줄이 안 들면 목록이 원작보다 짧다.
            Assert.GreaterOrEqual(Mathf.FloorToInt(boxH / row0.rect.height), 10,
                "목록 상자에 열 줄이 안 든다(행이 부풀었다) — 상자 " + boxH.ToString("0") + " / 행 " + row0.rect.height.ToString("0"));
