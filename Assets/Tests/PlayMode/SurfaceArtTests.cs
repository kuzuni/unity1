...
namespace Forge.Tests.PlayMode
{
    ...
    public class SurfaceArtTests
    {
        ...
        public IEnumerator 채팅_띠와_상점_카드와_승천_버튼과_펫_리본에_겹이_선다()
        {
            ...
            {
                bool allBaked = true;
                foreach (RectTransform rt in sp.Root.GetComponentsInChildren<RectTransform>(false)) if (kindOf(rt) != 0 && rt.Find("bg-grad") == null) { allBaked = false; break; }
                if (allBaked) break;
                yield return null;
            }
            int deals = 0, gems = 0;
            foreach (RectTransform rt in sp.Root.GetComponentsInChildren<RectTransform>(false))
            {
