...
namespace Forge.Core.Ui
{
    ...
    /// T121: 게임 글꼴은 그 기본이 아니라 `Resources/UiFontBake.json`(54pt · 패딩 9 → G 10)으로 굽는다 — 정본 최대 획 .2em(바깥 .1em)이
    /// 기본(90pt · 패딩 9 = 글자의 10%)에서는 정확히 천장(W = 1)이라 링이 «면» 이 됐다. 표 값이면 36px 글자의 최대 바깥 띠가 6px(W .6).
    /// 패딩을 15 로 키우는 길은 런 234 에서 띠가 이 식의 1.77배로 나와 버렸다(G = 패딩+1 이 실제 램프와 안 맞는다) — 식이 검증된 패딩 9 를 두고 샘플링을 내렸다.
