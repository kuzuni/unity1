...
namespace Forge.Core.Ui
{
    ...
    /// T121: 게임 글꼴은 `Resources/UiFontBake.json`(54pt · 패딩 9)으로 굽고, 재질 G 는 TMP 기본(패딩+1)이 아니라 **굽은 직후 아틀라스에서 잰 램프**(`UiFont.MeasureAlphaTexels` · 54pt·패딩 9 → 19.9 텍셀)로 세운다 —
    /// TextCore 동적 SDF 의 램프는 굽기 값에 따라 패딩+1 과 다르다(90pt·9 → 10 · 54pt·9 → 20 · 90pt·15 → 30 · 런 200·239·234). TMP 기본 G 로는 이 식의 «G 텍셀» 이 실제의 절반이라
    /// 링이 두 배 두꺼웠고(런 234·239 · 9px ↔ 5.08), 정본 최대 획 .2em 은 기본 굽기에서 W = 1(면)이었다. 지금 굽기면 36px 글자의 최대 바깥 띠 12px(.2em 은 W .3).
