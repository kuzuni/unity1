...
namespace Forge.Core.Ui
{
    ...
    /// T121: 게임 글꼴은 `Resources/UiFontBake.json`(54pt · 패딩 9)으로 굽고, 재질 G 는 TMP 기본(패딩+1 = 10)이 아니라 **실측 램프**(표 `alpha_texels` = 20 = 2×(패딩+1))로 세운다 —
    /// 런 239 `t121-ramp.txt` 가 아틀라스 «I» 행에서 알파가 텍셀당 12.8/255 오르는 것(= 19.9 텍셀/알파)을 읽었다. TMP 기본 G 로는 이 식의 «G 텍셀» 이 실제의 절반이라
    /// 링이 두 배 두꺼웠고(런 234·239 · 9px ↔ 5.08), 정본 최대 획 .2em 은 기본 굽기에서 W = 1(면)이었다. 표 값이면 36px 글자의 최대 바깥 띠 12px(.2em 은 W .3).
