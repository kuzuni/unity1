    ...
    public static class ItemFaces
    {
        ...
        static Sprite Bake(GameDefs defs, ForgeItem it, string k)
        {
            ...
            try
            {
                ...
                Outline(tex, ItemFacesStyle.L("cell_canvas_px"));
                tex.Apply();
                tex.name = "itemface:" + k;
            }
            catch (Exception ex)
        ...
        /// 두께: `sw_css_px × css_px`(정본 CSS px → 기준 캔버스 px · 결정 222) × `텍스처 한 변 / cellCanvasPx`(굽기 텍스처가 칸보다 크다) — 화면에 서면 정본과 같은 1.32 CSS px 다.
        /// 알파가 있는 곳은 건드리지 않는다(모델이 위다) — 빈자리에만 칠하므로 «모델을 덮는» 일이 없다.
        /// 2회차: <see cref="PetFaces"/> 도 같은 테를 쓰므로(정본 7663 `.pet-tile .tile-face .mt-face`·`.petd-tile .mt-face`·`.sk-mini .mt-face`)
        /// «이 썸네일이 서는 칸의 기준 캔버스 폭» 을 인자로 받는다 — 장비 칸과 펫 타일은 폭이 다르고 두께는 **화면에서** 같아야 한다.
        /// </summary>
        internal static void Outline(Texture2D tex, float cellCanvasPx)
        {
            int w = tex.width, h = tex.height;
            if (cellCanvasPx <= 0f) return;
            int sw = Mathf.RoundToInt(ItemFacesStyle.L("sw_css_px") * KeylineUi.CssPx * w / cellCanvasPx);
            if (sw <= 0) return;
