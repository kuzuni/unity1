...
- 🔄 2026-09-14 10:1x 워커 P(sess-0154-10159) **4회차 = ⓑ**: 목록 `fl-face`(정본 2183 hydrateForgeThumbs 그대로 — 렌더마다 새 작업 · 펌프 · rarity common · 티어 없음 · 장신구는 실루엣) · 상세 `idet-icon`(2244 · 동기 한 장) · `CraftCard`(buildCraftCard → itemImgHTML · ForgeItem 오버로드) · PlayMode `ItemFacesTests` +1. **판정(런 + `screen_forge-list` 투구 줄 다섯이 서로 다른가)은 다음 회차 · lock 쥔 채**.
