...
- 🔄 2026-09-14 04:5x 워커 T(sess-0444-16036) 2회차: T87 33회차 반납으로 `ForgeCraftPopup`·`ForgeInfoPopup` 이 열렸다 — 호출부 넷(판매 둘 · 건너뛰기 · 업그레이드)을 `IconTextStack` 으로 갈아 끼우고(아이콘은 이모지 표 길 · 결정 313) `LABEL_KNOWN` 넷을 뺀다 · 단언은 `IconTextStackTests`(`ForgeUiTests.cs` 는 T98 lock).· lock 반납 · **2회차 대기**(누구든 · T87 뒤).
