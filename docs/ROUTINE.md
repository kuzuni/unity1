...
- **24회차(2026-09-15 02:3x · 워커 N · sess-0524-8791)**: 축 = **`pointer-events` 68 선언**(none 67 · auto 1). 갈래 넷(전면 연출 층 13 · 소환 결과 연출 31 · 장식 겹 15 · 버튼 안 겹 8)을 클론에 짝지었다 — **결함 0**: 클론은 장식 공장(`UiKit.Panel`·`Icon`·`Text` …)이 `raycastTarget=false` 를 기본으로 두고 누르는 면(hit·모달 딤·뷰포트)만 켜므로 정본의 «기본 auto · 67 을 none» 과 **부호가 반대로 같은 결과**다. `.skill-btn.empty` 는 `UiKit.Box`(버튼 없음) ✓ · 버튼 안 겹(`sk-cd`·`cell-img`)은 유니티에서 클릭이 부모로 올라가 뜻이 없다. 정책을 화면에서 재는 자 **`PointerPassTests`**(PlayMode 3 — 평시·보스 경고 한가운데·토스트 위에서 탭 버튼 중심 레이캐스트의 맨 위 = 버튼 · 연출 층 0건)를 세웠다(판정 다음 런). 기록 `docs/parity.md` 24회차. 남은 축: `mix-blend-mode` 18 · `white-space` 41 · `background-position/size` · `cursor`(닫아도 됨).
