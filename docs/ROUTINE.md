...
- 옮기며 밟은 함정 셋(T87 1~5회차 실측 · 정본 주석이 먼저 적어 둔 것들):
  ...
  - ⓓ **각진 SVG 면은 둥근 사각으로 옮길 수 없다** — 빌릿은 8각 챔퍼 각봉이어야 하고(정본 주석: 둥근 알약은 UI 어휘) 몸통은 4-stop 그라디언트다. `Ui/CraftFxPoly.cs`(정규 좌표 폴리곤 + 가로 띠 그라디언트 + `Opacity`)를 쓴다 — 오토포지 망치·불티·링도 같은 자로(결정 223).
