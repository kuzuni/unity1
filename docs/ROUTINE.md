...
### T380 — 대장간 정보 팝업의 **시대 막대 무늬 마스크가 없다**: 정본 `.fi-age-bar::before` 는 왼쪽 24% 투명 → 46% 불투명인데 클론은 자동 제련 막대(30→50%)만 마스크한다 (UI · T124 갈래 · **T33 33회차 등재**)
...
### T382 — 제작 묶음 카드(`.craft-batch .cb-card`)의 **잉크 비율이 .9 로 코드에 박혀** 있다: 정본은 슬롯과 같은 fit-ink **.76** 이다 (UI · T19·T122 갈래 · **T33 33회차 등재**)
...
- 🔄 2회차 2026-09-15 12:2x 워커 J(sess-1218-33417 · 선점) **배선**: `SkillPanel.cs` 가 T368 3회차 반납으로 열렸다(산 lock 어느 «범위» 에도 없다 · `check_claim_scope` 로 확인). `Render()` 의 머리·배너 뒤 `y += bh + gap` 를 걷고 **`y = PetSkillStyle.Px("sk_grid_top_h") − PetSkillStyle.Px("pad_rem")`** 한 줄 — 표값은 **화면 위끝** 기준이고 이 패널(`panel-skills`)은 시트 `body` 가 `pad_rem` 만큼 내려 앉히므로 그만큼 뺀 자리가 지역 y 다(패널 자체의 위끝은 화면 0 — `SkillPetSheet.Build` 의 `panelH = tabbar_top × RefH`). 머리·배너 높이는 안 건드렸다(§2 대로 «틈만»).
  ...
  - ⚠ **하마터면 «안 고쳐졌다» 로 읽을 뻔했다**: 레포의 `ui-screens/screen_skills.png` 는 **이틀 묵은**(09-13 17:18) 파일이라 `--rows` 가 종전 −1.9·−1.8 을 그대로 뱉었다. 판정 PNG 는 **그 런의 `screens` 브랜치에서 새로 받아** 덮어쓴 뒤 재야 한다(결정 650).
...
| `ref/screens/shot-*.png` 30장 · `tools/shot-*.js` · `ref/UI-SPEC.md` · `ref/POLISH.md` | 원작 화면 정본 · 촬영 도구 · 비율 규격 | T27(촬영) · T28(대조) · T33(완주) · T77(촬영 시드 전투력) · T83(촬영 두 장 가르기) · T128(촬영 상태 결정론) · T166(정본 화면별 상태 주입을 펫 넷이 안 따라 한다) · T176(촬영이 매 런 같은 자리에서 보스 경고 연출을 문다 — gear-detail 씬 대역 84%) | T27 ✅(원작 30장 전부 열림 + `screen_*.png` 31장 + 짝 표 `screens.json` · CI 런 83) · T28 🔄 · T33 🔄 · T77 ✅ · T83 ✅ · T128 ✅ · T166 ✅ · T176 ✅ |
