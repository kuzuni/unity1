...
- **ⓒ 갈래 — 촬영이 «카드 팝» 도중에 찍힌다(2026-09-14 · T28 38회차 · 워커 M · 런 341)**: 팝업 여섯 화면이 **반투명·축소 중**인 채로 찍혔다(`screen_autoforge.png` 을 열면 카드 너머 세계·장비 칸이 그대로 비친다). 자 지문도 같은 말을 한다 — «팝업 안» 차가 `profile` **64.3** · `autoforge` **55.1** · `offline` **45.4** 이고 밴드가 `autoforge` 21 → **14**, `autoforge-filter` 19 → **12** 로 무너졌다(평균 4.36 → **4.17**).
...
- 🔄 2026-09-14 06:5x 워커 N(sess-0524-8791) 2회차 = ⓑ: `PopupLayer.Show` 의 새 팝업 갈래가 뿌리에 `CardPop` 러너를 한 번 붙이고(재호출은 안 붙는다 = ui.js 1156), 러너가 카드 `"card"` 를 매 프레임 찾아 표(`CardPopUi.json` · `CardPopSpec` · .25s ease-out · scale .7→1 · α 0→1)대로 걸고 끝나면 원래 모습으로(결정 324) · EditMode 3 · PlayMode 1. 판정은 다음 런(팝은 정지 촬영에 안 찍혀 PlayMode 단언이 본다). ⓒ2 는 T122 lock 뒤.
