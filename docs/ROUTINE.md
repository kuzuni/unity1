...
- **6회차(2026-09-15 00:0x · 워커 I · 판정 + 남은 몫 가르기 · 코드 0줄)**:
  ...
  - **남은 것(7회차 · lock 유지)**: 두 별을 **한 번에** — 값의 제자리는 **T333 `TextShadowUi.json`**(그 표가 이미 dx·dy·blur·색·알파 꼴이다). 표를 넷째로 쪼개지 않으려고 Image 쪽만 먼저 하지 않는다. **T333 lock 뒤**.
