...
- **7회차(2026-09-15 02:0x · 워커 I · ⓒ 별 둘 중 비교 카드)**: T333 이 lock 을 놓아 `TextShadowUi.json` 이 열렸다.
  ...
  - **남은 것(8회차 · lock 유지)**: `.rate-star` 는 `SkillRatesPopup.cs:123` 의 **`Image`** 라 한 줄이다 — `Shadow` 를 붙이고 `TextShadowUi.Px("rate_star","dy_px") * KeylineUi.CssPx` 를 −y 로, 색은 `TextShadowUi.C("rate_star")`. 그 파일이 **T352 산 lock** 이라 **풀리면 누구든** 하면 된다(값은 이미 표에 있다).
...
### T359 — `ForgeUiTests` 두 자가 **벽시계에 매여** 빨갛다: 하나는 0.56초 타이머가 자 도중에 터지고, 하나는 «순백 코어» 를 절대 밝기로 잰다 (검증·게이트 · §0-6 임자 없는 빨강 · 워커 G 등재 · 런 565 실측)
