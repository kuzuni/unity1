...
- 🔄 **4회차 2026-09-15 05:2x 워커 B(sess-1920-15773 · 선점)**: 남은 21 중 지금 산 lock 없는 파일의 자리 둘 — 채팅 말풍선(`ChatScreen.cs` · `chat_bubble_lh_w` = calc(app-w×.0351) · 앱 폭 키의 유일한 자리) · 탈것 업그레이드 팝업 빈 글(`MountUpgradePopup.cs` · `mat_grid_mat_empty_lh` 1.35) + PlayMode 칸 둘(05:4x push · 말풍선은 긴 말을 보내 두 줄로 꺾어 실제 줄 간격을 잰다 · 판정은 다음 런 `LineHeightTests` 4).
  ...
  → **런 609 판정(6회차 · 07:0x · 워커 B)**: `LineHeightTests` 5/5 PASS. T359 반납으로 열린 `PetUpgradePopup` 빈 글(`mat_grid_mat_empty_lh` 1.35) 한 줄 더 + 자 1 → 판정은 다음 런 `LineHeightTests` 6. 나머지 17 은 T331·T332·T333·T334 lock 뒤. 나머지 19 는 T178(sheet-sub 셋·league 둘·pinfo)·T331(swc-name·sellwarn-note·sk-mini·skd-*·chat-preview)·T332(idet-*·cmp-sub)·T334(sr-*)·T359(petup mat-empty) lock 뒤 · `gate.sh` 등록은 T331 «범위» 에 그 파일이 있어 뒤 번호가 기다린다.
