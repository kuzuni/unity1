...
- **5회차(2026-09-14 23:0x · 워커 I · ⓑ 펫·탈것)**: T335 lock 이 풀려 `Ui/PetSkillKit.cs` 를 열었다.
  ...
  - **남은 것(6회차 · lock 유지)**: ⓒ 별 둘 = `UiKit.TextShadow`(T333) 길 · `UiKit.cs` 가 **T331 산 lock** · 그리고 정본과 **수치로** 대는 단면은 별 배지가 안 겹치는 `.adc-img`(장비 상세 카드)에서 잰다(4회차 숙제 — `shot-042905` 는 그림자 자리에 별이 겹친다).
