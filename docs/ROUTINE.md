...
### T189 — 부팅 콘솔 빨강 하나가 런 375 부터 계속 서 있다: 손상 대기품 세이브가 `ForgeUi.AgeColor` 를 한 번 더 울린다 (등재만 · 임자 없음 · T19·T157 뒤 · 워커 J 등재)
- 실측(런 438 `playmode-red.txt` 53~59): `RED  Error  [BootGuardTests.손상_대기품_세이브로…]` — **테스트는 PASS** 이지만 §1 «플레이 콘솔 에러 0» 이 깨져 있다. 자취가 대는 자리는 `Assets/Scripts/Game/Ui/ForgeHost.cs:422`(`RestorePendingCraft` ← `BootGuard` ← `Boot`)이고, 울리는 것은 `[ForgeUi.AgeColor] 시대표에 없는 시대 키다(손상 세이브?)` 다. **런 375·383·418·438 에 계속 있다.**
- 왜 남았나: T157(격리)은 ✅ 로 닫혔고 그 자가 **일부러** 손상 세이브를 먹이는 자리라 워커들이 «내 것이 아니다» 로 지나쳐 왔다(이 표 2410·2430·4714줄 · 세 사람이 손으로 적었다). 임자가 ✅ 라 아무도 안 잡는 틈이다. 자 쪽 구멍은 T188 이 닫았고(이제 자가 `ForgeHost.cs` 를 가리킨다) **빨강 자체가 남았다**.
- 고치는 법은 이미 나와 있다(이 표 23줄 · 워커 H 가 T157 임자에게 적어 둔 것): ⓐ 그 줄도 `LogAssert.Expect` 하거나 심는 대기품에 **시대 키를 넣는다** ⓑ **덧(중요)**: 빨강이 뜨는 순간 본문이 끊겨 끝의 `h.StopAutoSeq(); DeleteSave();` 가 **안 돈다** → 손상 세이브가 디스크에 남아 뒤따르는 PlayMode 자들이 그걸 물고 부팅한다. `[UnityTearDown]` 으로 옮기면 그 그림자가 없어진다.
- 판정: 다음 유니티 런의 `playmode-red.txt` 에 그 `RED` 줄이 **없다** + `BootGuardTests` 둘 다 PASS 유지.
- 범위: `Assets/Tests/PlayMode/BootGuardTests.cs`(**각 파일 lock 뒤** · 잡는 사람이 PROGRESS «범위» 칸에 적는다) · 필요하면 `Assets/Scripts/Game/Ui/ForgeHost.cs`.
