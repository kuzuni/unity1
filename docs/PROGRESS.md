...
- **게이트**: `node tools/export_data.js --self-test` 통과(27항목) · `tools/check_data_sync.sh .wwwww-src` ✓ · `gen_meta.py --check` ✓ · `check_docs_intact`·`check_decisions`·`check_task_rows`·`task_state --check`·`check_claim_scope` 전부 rc 0. **이 컨테이너에 `dotnet` 이 없다** — C# 은 한 줄도 안 바꿨으므로 CI `dotnet` 잡으로 확인한다(런 번호는 lock 반납 커밋에서 채움).
