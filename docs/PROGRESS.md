...
- **게이트**: 컨테이너에 `dotnet` 없음(설치도 프록시 403) → `dotnet build/test` 는 CI `dotnet` 잡으로 확인(런 번호는 lock 반납 커밋에). `gen_meta --check` · `gen_ui_catalog --check` · `check_docs_intact` · `check_decisions` · `check_task_rows` · `task_state --check` · `check_claim_scope` 초록.
