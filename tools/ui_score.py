...
MEASURED_NOTES = {
    ...
    u"tech-node": u"가로 상자는 **다 쟀고 T477 로 등재했다**(T28 141회차 · 런 1205): 진행바 폭 원작 65.8 ↔ 클론 **69.63%W**(+3.8%p) · "
                  u"청동 원 지름 10.18 ↔ **11.48%W**(+1.3%p) · 진행바 좌 인셋 5.50 ↔ **3.33%W**. 임자는 하나 — `.item-detail` 의 CSS %% 밑변 다섯 자리다. "
                  u"🚫 **[취소] 버튼이 없는 것과 파란 채움 0%% 를 결함으로 읽지 마라** — 앞은 정본이 일부러 없앤 것이고(위 «원작 샷이 다른 자리») 뒤는 남은 시간이 정하는 **상태**다",
