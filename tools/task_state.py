...
def verdict(tid, heads, rows):
    ...
    hn, hdone, htext = heads.get(tid, (0, False, ""))
    ...
    #   ⚠ 제목의 **머리**(맨 앞 글자 자리)에서만 읽는다 — `_fold` 가 표 칸에서 그렇게 읽는 것과 **같은 꼴**이다(T249).
    #     «어디에든 ✂ 가 있으면» 으로 읽으면 본문에 ✂ 를 **인용한** 살아 있는 절이 통째로 접힘이 된다(T172 가 그렇게 당했다).
    if _lead(htext or "").startswith("✂"):
        return False, "**접힌 일이다**(ROUTINE 제목이 ✂) — 이미 남이 고쳤거나 다른 번호로 옮긴 자리다 · 잡지 마라"
    if pmark == "⛔":
        return False, "**폐기·흡수된 일이다**(PROGRESS 상태가 ⛔) — 잡지 마라"
    ...
    # T367 ⓑ — **두 문서 어디에도 없는 번호**는 «깨끗한 새 번호» 가 아니라 **선점할 일이 아니다**.
    #   선점은 «등재된 일» 에만 하는 것이고, 새 일이면 `--new-id` 로 번호를 받아 **먼저 등재**한다(§0-5).
    #   (`T183` 이 그 꼴이었다 — 접으면서 절과 행을 통째로 지워 번호만 남았고, 자는 매 회차 그것을 후보로 내줬다.)
    #   ⚠ **맨 끝에 둔다** — lock·발자취가 있으면 그쪽 말이 먼저다(번호가 문서에 없어도 누가 쥐고 있을 수 있다).
    if hn == 0 and pn == 0:
        return False, ("**등재된 적이 없는 번호다**(ROUTINE §2 에도 PROGRESS 표에도 없다) — 선점할 «일» 이 없다. "
                       "새 일이면 `--new-id` 로 번호를 받아 **먼저 등재**하고 잡는다(§0-5).")
...
def self_test():
    ...
    try:
        ...
        if done and verdict(done[0], H, R)[0]:
            ...
            return 1
        # T367 — **«쓰인 적 없는 번호» 의 답이 바뀌었다.** 예전엔 «깨끗하다 — 선점해도 된다» 였는데,
        #   선점은 «등재된 일» 에만 하는 것이라(§0-5: 이상이 있으면 **먼저 등재**하고 잡는다) 이제
        #   «등재된 적이 없는 번호다 — `--new-id` 로 받아 먼저 등재해라» 로 답한다. 거짓 경고가 아니라 **길 안내**다.
        #   («등재된 clean 번호는 그대로 잡아도 된다» 는 위 ⓥ 의 T901 칸이 지킨다.)
        _ok_free, _why_free = verdict(free, H, R)
        if _ok_free or "등재된 적이 없는 번호다" not in _why_free:
            print("⛔ 자기 검사 실패(T367) — 쓰인 적 없는 %s 는 «등재된 적이 없다» 로 안내해야 한다: %r / %s"
                  % (free, _ok_free, _why_free))
            return 1
        ...
        for (pm, lk_, age), want in t186:
            ...
            if got != want:
                ...
                return 1
        # ⓥ T367 — ✂ 로 접힌 절 · 두 문서에 없는 번호는 «선점 대상» 이 아니다
        heads367 = {
            "T161": (10, "", "✂ — 이미 남이 고쳤다 (검증 · 접었다)"),
            "T900": (11, "", "살아 있는 절 — 접을 때는 본문처럼 ✂ 를 인용해도 접힘이 아니다"),
            "T901": (12, "", "멀쩡한 절"),
        }
        rows367 = {"T901": (20, "⬜", "| T901 | 일 | ⬜ 대기 |"), "T902": (21, "⛔", "| T902 | 일 | ⛔ 폐기 |"),
                   "T900": (22, "⬜", "| T900 | 일 | ⬜ 대기 |")}
        t367 = [("T161", False, "접힌 일이다"), ("T900", True, None), ("T901", True, None),
                ("T902", False, "폐기·흡수된 일이다"), ("T903", False, "등재된 적이 없는 번호다")]
        for tid367, want_ok, want_why in t367:
            got_ok, got_why = verdict(tid367, heads367, rows367)
            if got_ok != want_ok:
                print("⛔ 자기 검사 실패(T367) — %s: %r (기대 %r) · %s" % (tid367, got_ok, want_ok, got_why))
                return 1
            if want_why and want_why not in got_why:
        ...
        # ⓩ **T403 — 두 등급의 종료 코드를 박아 둔다**: ⚠ 참고(제목은 닫혔는데 행은 열림 · T376 갈래)만 있으면 `--check` 는 **rc 0**,
        #    ⛔ 선점 덫(행은 닫혔는데 제목에 표시가 없다 · T161 갈래)이 하나라도 있으면 **rc 1**. gate.sh 가 이 자를 `block` 으로 올렸으니
        #    (종전 `report`) 이 두 등급이 바뀌면 «⚠ 만으로 main 이 막히거나 · ⛔ 가 조용히 지나가는» 사고가 된다 — 자가 스스로 지킨다.
        import contextlib
        _sink = io.StringIO()
        io.open(r, "w", encoding="utf-8").write("### T161 ✅ — 촬영 카메라\n")
        io.open(p, "w", encoding="utf-8").write(
        ...
        _sink = io.StringIO()
        with contextlib.redirect_stdout(_sink):
            _rc_trap = cmd_check(routine_heads(r), progress_rows(p))
        if _rc_trap != 1 or "선점 덫" not in _sink.getvalue():
            print("⛔ 자기 검사 실패(T403) — ⛔ 선점 덫(행 ✅ · 제목 표시 없음)인데 --check 가 rc %s 로 안 막았다" % _rc_trap)
            return 1
