...
TABLE_INK = {
    ...
    '.asc-wipe-warn': ['Ui/AscendPopup.cs|res:PinnedColorUi:asc_wipe_warn_ink'],     # 5637 #ff6b5e — 승천 효과 글줄 가운데 줄(소멸 경고) · 3차(결정 797)
...
KNOWN_INK = {
    ...
    # T396 20회차 — `.asc-wipe-warn` 은 1차(런 1180)에 «<color> 태그를 켜자 네 줄로 접혔다» 로 KNOWN 에 뒀다가 3차에 TABLE 로 올렸다: 태그 없는 런 1186 도 네 줄이라
    #   원인은 T473 폭 + NotoSansKR 폭 차(결정 796 · 정본 패딩 .5rem 복원)였고 태그는 줄 폭을 안 바꾼다(결정 797). BrLinesTests 는 «둘째 항목 안 한 번 접힘» 까지 허용한다.
